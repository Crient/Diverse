...
const renderToCard = (data, renderArea) => { 
    ...
    <div class="col-4 col-sm-4 col-md-3 col-lg-2">
            <a href="movies-details.html?${movie.id}" class="text-decoration-none">
             <div class="card g-5 h-100 shadow">
...
const fetchPopularMovies = () => {
    console.log(`${baseURL}movie/popular?api_key=${API_KEY}&page=${page}`)
...
loadMoreBtn.addEventListener("click", getUSerInfo);
 ...
 function getUSerInfo() {
    ...
    console.log(`${baseURL}movie/popular?api_key=${API_KEY}&page=${nextPage}`)
    fetch(`${baseURL}movie/popular?api_key=${API_KEY}&page=${nextPage}`)
    .then(resp => resp.json())
    .then(movies => {renderToCard(movies.results, ".popular-movies");
...
/*
 This method too, no need to call it twice 
 because it already call in load button; 
      getUSerInfo();
 */
// getUSerInfo();
fetchPopularMovies();